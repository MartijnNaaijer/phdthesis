...
dat.jc.nojc <- datjc.nojc[, selected_cols]
...
xgb_cols <- c("cl_type", "ebh_lbh", "genre", "txt_type"  ,"s_p_order" ,"CP", 
              "cl_len", "subj_len", "pc_len", "main_sub", 
              "Loca", "Ques", "Rela", "Time", "mother") 
...
CV.xgb <- lapply(1:nfolds, function(x){ 
  ...
  # train the xgb model
  model_xg <- xgboost(data = train, eta = 0.1, label = label_train, maxdepth=4, nrounds=400, objective='binary:logistic')
